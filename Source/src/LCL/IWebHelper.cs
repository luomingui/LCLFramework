...
namespace LCL
{
    ...
    public partial class WebHelper : IWebHelper
    {
        ...
        public virtual T QueryString<T>(string name)
        {
            ...
            if (IsRequestAvailable(_httpContext))
            {
                if (_httpContext.Request.Form[name] != null)
                {
                    queryParam = _httpContext.Request.Form[name];
                }
                if (string.IsNullOrWhiteSpace(queryParam))
                {
                    if (_httpContext.Request.QueryString[name] != null)
                        queryParam = _httpContext.Request.QueryString[name];
                }
                if (string.IsNullOrWhiteSpace(queryParam))
                {
                    if (_httpContext.Request.Cookies[name] != null)
                        queryParam = _httpContext.Request.Cookies[name].Value;
                }
            }
            if (!String.IsNullOrEmpty(queryParam))
                return Utils.To<T>(queryParam);
        ...
        public virtual void RestartAppDomain(bool makeRedirect = false, string redirectUrl = "")
        {
            ...
            else
            {
                ...
                if (!success)
                {
                    throw new LException("nopCommerce needs to be restarted due to a configuration change, but was unable to do so." + Environment.NewLine +
