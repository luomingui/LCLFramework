...
{
    /// <summary>
    /// 插件管理的主类，看里面的注释，它的插件机制应该是参考的Umbraco这个cms的。
    /// </summary>
    public class PluginManager
    {
            ...
            using (new WriteLockDisposable(Locker))
            {
                ...
                try
                {
                    ...
                    foreach (var dfd in GetDescriptionFilesAndDescriptors(pluginFolder))
                    {
                        ...
                        if (!pluginDescriptor.SupportedVersions.Contains(LVersion.CurrentVersion, StringComparer.InvariantCultureIgnoreCase))
                        {
        ...
        private static Assembly PerformFileDeploy(FileInfo plug)
        {
            ...
            if (Utils.GetTrustLevel() != AspNetHostingPermissionLevel.Unrestricted)
            {
