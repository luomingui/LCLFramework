...
{ 
    ...
    ///  根据关键字进行查询 
    /// </summary> 
    public class KeyHM_ClientChargeSpecification : Specification<HM_ClientCharge> 
    { 
        string _keyword = ""; 
        string _fieldName = ""; 
        public KeyHM_ClientChargeSpecification(string keyword, string fieldName) 
        { 
            _keyword = keyword; 
            _fieldName = fieldName; 
        } 
        public override Expression<Func<HM_ClientCharge, bool>> GetExpression() 
        { 
            Expression<Func<HM_ClientCharge, bool>> spec = null; 
            switch (_fieldName) 
            { 
                case "ID": // 编号
