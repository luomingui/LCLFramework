...
namespace LCL.WebMvc.Controllers
{
    ...
    public class AccountController : CommonController
    {
        ...
        public ActionResult Register(UserAccountModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    (Membership.Provider as LCLMembershipProvider).CreateUser(model.UserName,
                        model.Password,
                        model.Email,
                        null,
                        null,
        ...
        {
            using (var proxy = RF.Service<IUserService>())
            {
                var dataObject = model.ConvertToDataObject();
                proxy.UpdateUsers(new List<User> { dataObject });
